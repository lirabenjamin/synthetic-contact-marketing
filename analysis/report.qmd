...
While participants accurately perceived their ingroups (Democrats: *M*~actual~ = `r sprintf("%.2f", dem_ingroup_actual)`, *M*~perceived~ = `r sprintf("%.2f", dem_ingroup_perceived)`, *p* `r pformat(dem_ingroup_p)`; Republicans: *M*~actual~ = `r sprintf("%.2f", rep_ingroup_actual)`, *M*~perceived~ = `r sprintf("%.2f", rep_ingroup_perceived)`, *p* `r pformat(rep_ingroup_p)`), they systematically misperceived their political outgroups.
Democrats significantly underestimated Republicans' environmental concern (*M*~actual~ = `r sprintf("%.2f", dem_outgroup_actual)`, *M*~perceived~ = `r sprintf("%.2f", dem_outgroup_perceived)`, *d* = `r sprintf("%.2f", dem_outgroup_d)`, *p* `r pformat(dem_outgroup_p)`).
